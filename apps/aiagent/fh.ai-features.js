...
const QR_SYSTEM_CONTEXT = [
    ...
    '二维码 AI 任务边界：1）把自然语言转换成标准二维码载荷，例如 Wi-Fi、vCard、VEVENT、SMSTO、tel、mailto、geo、URL；2）识别解码文本的类型、关键字段和风险；3）根据内容长度、颜色、尺寸、icon 和条形码格式做扫码可靠性体检。',
    '回答策略：不要泛泛解释二维码是什么；优先输出可复制、可应用、可验证的载荷文本。生成任务必须把最终载荷放进 ```text 代码块；解码任务必须先判断类型，再列风险和建议。',
...
const FORMAT_SYSTEM_CONTEXT = [
    ...
    '回答策略：优先解释代码意图、指出格式/可读性/潜在风险，再给出更清晰的版本；不要只说“可以美化”，要直接给可操作结果。',
...
const AI_FEATURE_PACKS = [
    {
        toolKey: 'json-format',
        badge: 'JSON',
        entryTask: 'repair-json',
        title: 'JSON 错误修复',
        desc: '解析失败时解释错误并给出可应用的合法 JSON。',
        prompt: '请启动 FeHelper JSON 自动化模式。基于已加载的 FeHelper JSON 工具能力，简洁说明你能帮我完成哪些 JSON 自动化任务，并告诉我接下来应该粘贴什么内容。',
    ...
    },
    {
        ...
        title: '二维码智能载荷',
        desc: '生成标准载荷、识别解码内容并检查扫码可靠性。',
        prompt: '请启动 FeHelper 二维码智能模式。基于已加载的 FeHelper 二维码工具能力，简洁说明你能帮我生成哪些标准载荷、识别哪些解码内容，并告诉我接下来应该提供什么。',
