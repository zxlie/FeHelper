...
FeHelper 是一款强大的浏览器开发者工具集（v2026.04），为Web开发者提供了丰富的开发辅助功能。它集成了32+种实用工具，包括代码格式化、JSON处理、编码转换、UUID/雪花ID生成、二维码/条形码、Gzip压缩、性能分析、AI助手等，旨在提高开发效率，简化开发流程。

## 主要特性

- 🛠️ **丰富的工具集**：包含32+种开发工具，涵盖前端开发的方方面面
...
| 工具名称 | 功能描述 | 
|---------|---------|
| [信息编码转换](?tool=en-decode) | 支持 Unicode、UTF-8、UTF-16、URL、Base64、MD5、Hex、JWT 解码，v2026.04 新增 Gzip 压缩/解压、字符串转义/去转义 |
| [进制转换工具](?tool=trans-radix) | 支持2~36进制任意互转，v2026.04 升级为 BigInt 引擎，支持任意精度大数运算 |
| [时间(戳)转换](?tool=timestamp) | 本地化时间与时间戳互转，支持秒/毫秒、世界时区切换，v2026.04 新增 Windows FILETIME 转换 |
...
| 工具名称 | 功能描述 | 
|---------|---------|
| [二维码/条形码](?tool=qr-code) | 二维码生成/解码 + v2026.04 新增条形码生成（Code128/EAN-13/EAN-8/UPC/ITF-14），支持 SVG/PNG 导出 |
...
| [UUID/ID生成器](?tool=uuid-gen) | v2026.04 全新工具！生成 UUID v4、Twitter Snowflake 雪花 ID、NanoID，支持批量生成与雪花 ID 解析 |
...
- **编解码转换类**：各种格式的数据转换和编解码工具（含Gzip/FILETIME/BigInt）
- **图像处理类**：二维码/条形码、图片转换、截图等图像相关工具
- **效率工具类**：AI助手、便签、Markdown、思维导图等提升效率的工具
- **计算工具类**：UUID/ID生成器、Crontab、贷款计算、密码生成等
- **其他工具**：Excel转换、网页栅格标尺、FH开发者工具
...
### v2026.04 新版亮点
- 🔐 **安全加固**：全面移除 evalCore 动态代码执行，消除 XSS 注入风险，添加输入类型与长度守卫
- ❄️ **UUID/ID生成器**：全新工具！支持 UUID v4、Twitter Snowflake 雪花 ID、NanoID 批量生成与解析
- 📊 **条形码生成**：二维码工具升级，新增 Code128/EAN-13/EAN-8/UPC/ITF-14 等条形码格式
- 🗜️ **Gzip 压缩/字符串转义**：编解码工具新增 Gzip 压缩/解压、字符串转义/去转义
- ♾️ **BigInt 精度升级**：进制转换改用 BigInt 引擎，支持任意精度大数运算
- ⏰ **FILETIME 转换**：时间戳工具新增 Windows FILETIME 互转
- 🧪 **自动化测试**：引入 Vitest 单元测试框架，79+ 测试用例 + GitHub Actions CI
